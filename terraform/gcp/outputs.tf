...
output "lb_domain" {
  ...
  description = "Domain on the managed TLS certificate: var.domain_name when set, otherwise a nip.io fallback"
}
...
output "aws_elastic_ip" {
  value       = var.aws_eip
  description = "AWS Elastic IP the Internet NEG points at directly"
}
